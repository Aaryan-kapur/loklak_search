...
export class UserInfoBoxComponent implements OnInit {
	...
	ngOnInit() {
		this.sortedApiResponseUserFollowing = immutableSort(
			this.apiResponseUserFollowing,
			this.compareUsers
		);
		this.sortedApiResponseUserFollowers = immutableSort(
			this.apiResponseUserFollowers,
			this.compareUsers
		);
	}

	private compareUsers(user1: UserApiResponse, user2: UserApiResponse): number {
		const follower_count_diff: number =
			user1.followers_count - user2.followers_count;
		const statuses_count_diff: number =
			user1.statuses_count - user2.statuses_count;
